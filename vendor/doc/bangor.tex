...
Load the module with \verb|\usepackage{bangortables}|. It loads the
packages \textsf{longtable}, \textsf{tabularx}, and \textsf{pdflscape}.

\begin{itemize}
  \item Use \verb|longtable| for a table that can be longer than one page.
        The header repeats on each page if you put it before \verb|\endhead|.
  \item Use \verb|tabularx| with the \verb|X| column type for a table that
        must fit the text width.
  \item Put a wide table in a \verb|landscape| environment.
\end{itemize}
